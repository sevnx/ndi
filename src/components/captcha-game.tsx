...
interface GridItem {
  ...
  position: number;
  gridId: number;
  imageSrc: string; // New property for image source
}
...
const Item: React.FC<ItemProps> = ({
  ...
  return (
    ...
    >
      {item.imageSrc ? (
        <img src={item.imageSrc} alt={item.content} className="w-full h-full object-cover" />
      ) : (
        item.content
      )}
    </div>
...
export default function CaptchaGame() {
  ...
  useEffect(() => {
    // Create initial items for grid 1
    const grid1Items = Array.from({ length: 9 }, (_, i) => ({
      id: `item-1-${i + 1}`,
      content: `Item ${i + 1}`,
      position: i,
      gridId: 1,
      imageSrc: `/images/grid1/item${i + 1}.jpg`, // Path to grid 1 images
    }));

    // Create initial items for grid 2
    const grid2Items = Array.from({ length: 9 }, (_, i) => ({
      id: `item-2-${i + 1}`,
      content: `Item ${i + 1}`,
      position: i,
      gridId: 2,
      imageSrc: `/images/grid2/item${i + 1}.jpg`, // Path to grid 2 images
    }));

    // Get shuffled positions for both grids
    const shuffledPositions1 = shuffleArray([...Array(9).keys()]);
    const shuffledPositions2 = shuffleArray([...Array(9).keys()]);

    // Apply shuffled positions to grid items
    const shuffledGrid1Items = grid1Items.map((item, index) => ({
      ...item,
      position: shuffledPositions1[index],
    }));

    const shuffledGrid2Items = grid2Items.map((item, index) => ({
      ...item,
      position: shuffledPositions2[index],
    }));

    // Set the items with shuffled positions
